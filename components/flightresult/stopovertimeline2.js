...
if (!flightOffer) return <>Loading ...</>;
  ...
  useEffect(() => {
    if (inBoundSegments.length > 1) {
      let newStopsArray = [];
      for (let i = 0; i < inBoundSegments.length - 1; i++) {
        let cloneFirstSegment = { ...inBoundSegments[i] };
        delete cloneFirstSegment.departure;
        let cloneNextSegment = { ...inBoundSegments[i + 1] };
...
export default StopoverTimeline2;
